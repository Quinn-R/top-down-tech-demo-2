...
{
    if(direction == "up" && touchingGround == 1)
    {
        touchingGround = 0;
...
void character::gravity(std::vector< std::vector<collideObject> > vecCollideObjects)
{
    for(int i = 0; i < speed; i++)
    {
        updateBuffer();
        if(updateCollide("down", vecCollideObjects) == 0)
        {
            characterShape.move(0, 1);
        }
    }
